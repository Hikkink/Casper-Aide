      ...
      case 'w':
    	    ...
    	    if(this.player.hasItemTemplate(idbox,1)) {
    	        for (ObjectAction action : cad.getOnUseActions()) {
    	            ...
    	            try {
    	                ...
    	                player.sendMessage("ERROR: por favor, contacta a un miembro del personal.");
    	                break;
    	            }
    	            //
    	        ...
    	        player.sendMessage("Acción imposible: ya no tienes el "+Main.world.getObjTemplate(idbox).getName()+".");
    	        player.send("kn");
    	    }
    	    break;
		default:
		...
		case 'I': // buy ornements
            int idss = Integer.parseInt(packet.substring(2));
                    if(idss == 0) {
                        player.sendMessage("Acabas de desactivar tu Ornamento");
                    ...
                    if(player.getOrnementsList().contains(idss))
                        ...
                        if(player.getAccount().getPoints() < World.getOrnements().get(idss).getPrice()) {
                            player.sendMessage("¡No tienes suficientes puntos de tienda para comprar este Ornamento!");
                            break;
                        } else {
                            player.setOrnement(idss);
                            player.addOrnements(idss);
                            player.getAccount().setPoints(player.getAccount().getPoints() - World.getOrnements().get(idss).getPrice());
                            player.sendMessage("Acabas de comprar un nuevo adorno, te quedan <b> puntos de tienda"+player.getAccount().getPoints()+"</b> puntos de tienda.");
                        }
       ...
       // parseCustomPacket(packet);
  	    ...
  	    case 's':
  	    	 switch(packet.charAt(2))
  	  	    ...
  	  	    break;
				case '5':
					SocketManager.GAME_SEND_MESSAGE(player, "El servicio de título cuesta <b>" + Config.getInstance().prix_titre + "</b> puntos!", "000000");
					this.send("béé");
					break;
  	  case 'N':
    ...
    switch(packet.charAt(1))
    {
      ...
      case 'p':
    	    ...
    	    case 'c':
  	            ...
  	            SocketManager.GAME_SEND_MESSAGE(player, "Tus características han sido reiniciadas", "009900");
      		  break;
    	    case 's':
...
private void crearPacketKey() {
  ...
  private void boost(String packet)
  {
    try
    {
      ...
      {
        this.player.sendMessage("Estás transformado, no puedes usar tus puntos de características en este estado.");
        return;
      }
...
public void setTimeLastTaverne(long timeLastTaverne) {
    ...
    try
    {
      ...
      if(this.account==null )
      {
			...
			return;
				...
				System.out.println("No se pudo cargar la cuenta "+id);
			SocketManager.GAME_SEND_ATTRIBUTE_FAILED(this);
			chek = 0;
			this.kickv2();
			return;
  ...
  private void tchat(String packet)
  {
    ...
    if(this.player.getCurMap()!=null)
    {
      ...
      {
        this.player.sendServerMessage("El mapa está silenciado actualmente.");
        return;
      }
    }
    ...
    switch(packet.charAt(2))
    {
      ...
      case ':'://Canal commerce
        ...
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"No tienes permiso para usar este canal.","B9121B");
        }
        else if(this.player.isInPrison())
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"Estás encarcelado, no puedes usar este canal","B9121B");
        }
        else
        {
  ...
  private void buy(String packet)
  {
    ...
    if(player.boutique)
    {
      ...
      try
      {
        ...
        {
          this.player.sendMessage("Por favor, compra solo una mascota, \\u00e0 la fois !");
          return;
        }
        if (template.getType() == 85 && quantity > 1) {
			this.player.sendMessage("Por favor, compra solo una Gema Espiritual. \u00e0 la fois !");
			return;
		}
		if (template.getType() == 97 && quantity > 1) {
			this.player.sendMessage("Por favor, compra solo un Dragopavo \u00e0 la fois !");
			return;
		}
        ...
        if(points<0)// Si le joueur n'a pas assez de point
        {
          ...
          SocketManager.GAME_SEND_MESSAGE(player,"No tienes suficientes puntos para comprar. "+(quantity>1 ? "cet objet" : "cet objet")+", vous avez actuellement "+account.getPoints()+" points, il vous en faut "+diferencia+" de plus.","FF0000");
          SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
          return;
        }
        ...
        SocketManager.GAME_SEND_MESSAGE(player,"Acabas de comprar "+(quantity>1 ? "items" : "item")+": <b>"+template.getName()+"</b> para <b>"+value+"</b> points!","000000");
        SocketManager.GAME_SEND_MESSAGE(this.player,"Te quedan : " + this.account.getPoints() + " ¡Puntos de tienda!");
    ...
    else
    {
      ...
      try
      {
        ...
        {
          this.player.sendMessage("Por favor, no compres solo un animal de compañía, estaría solo.");
          return;
        }
        if (template.getType() == 85 && qua > 1) {
			this.player.sendMessage("Por favor, compra solo una Gema Espiritual. \u00e0 esta vez !");
			return;
		}
		if (template.getType() == 97 && qua > 1) {
			this.player.sendMessage("Por favor, compra solo un Dragopavo \u00e0 esta vez !");
			return;
		}
        if(npc==null)
          return;
        ...
        if (npcTemplate.getId() ==  1121 ||npcTemplate.getId() ==  1127) {
			...
			if (!player.hasItemTemplate(10275, value)) {
				SocketManager.GAME_SEND_MESSAGE(this.player,
						"No tienes suficientes puntos para comprar este artículo.");
				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
				return;
			}
        ...
        if(template.getPoints()>0&&(npcTemplate.getInformations()&0x4)==4)
        {
          ...
          if(points<value)
          {
            SocketManager.GAME_SEND_MESSAGE(this.player,"No tienes suficientes puntos para comprar este artículo. Actualmente tienes "+points+" puntos y necesitas "+(value-points)+" para comprar este ítem.");
            SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
            return;
          }
          ...
          SocketManager.GAME_SEND_Ow_PACKET(this.player);
          SocketManager.GAME_SEND_MESSAGE(this.player,
					"Acabas de comprar " + qua + (qua > 1 ? " items" : " item") + " : <b>"
							+ template.getName() + "</b> al precio de <b>" + value + "</b> Puntos!",
					"000000");
        SocketManager.GAME_SEND_MESSAGE(this.player,
					"Te quedan : " + this.account.getPoints() + " Puntos !");
        Database.getStatics().getPlayerData().shop_item(this.player.getName(), this.player.getAccID(), value, template.getId());
        if(player.getKamas() < 5000)
			SocketManager.GAME_SEND_STATS_BOUTIQUE_PACKET(player);
        ...
        else if(template.getPoints()>0)
        {
          ...
          if(points<value)
          {
        	  SocketManager.GAME_SEND_MESSAGE(this.player,"No tienes suficientes puntos para comprar este artículo. Actualmente tienes  "+points+" puntos y necesitas "+(value-points)+" para comprar este artículo.");
              SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
            return;
          }
    ...
    switch(packet.charAt(2))
    {
      ...
      //  if(player.getWorldMarket())
       ...
       // else
        ...
        if(hL==null)
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"Ocurrió un error al confirmar la compra. Por favor, contacta a un miembro del personal.");
          return;
        }
        ...
        if(hE==null)
        {
          ...
          SocketManager.GAME_SEND_MESSAGE(this.player,"[2 - Template '"+hL.getTemplateId()+"'] Ocurrió un error al confirmar la compra. Por favor, contacta a un miembro del equipo de soporte.");
          return;
        }
        Integer owner=hE.getOwner();
        if(owner==null)
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"[3 - Template '"+hL.getTemplateId()+"']Este objeto no tiene dueño. Por favor, contacta a un miembro del personal.");
          return;
        }
        if(owner==this.player.getAccount().getId())
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"Tu no puedes comprar estos items.");
          return;
        }
        ...
        switch(packet.charAt(2))
        {
          case 'O'://Objets
            if(packet.charAt(3)=='+')
            {
              ...
              {
                Main.world.logger.error("Error en la Tienda de Intercambio' "+packet+"' => "+e.getMessage());
                e.printStackTrace();
                return;
              }
            }
      ...
      case ExchangeAction.IN_MOUNT:
        ...
        switch(packet.charAt(2))
        {
          case 'O':// Objet
            ...
            {
              SocketManager.GAME_SEND_MESSAGE(this.player,"Error de inventario Dragopavo: el artículo no existe.");
              return;
            }
      ...
      case ExchangeAction.AUCTION_HOUSE_SELLING:
        ...
        for(String s : packet.substring(4).split("\\-"))
         ...
         String[] infos=s.split("\\|");
            ...
            {
              Main.world.logger.error("Error en el Mercado HDV '"+packet+"' => "+e.getMessage());
              e.printStackTrace();
              this.player.sendMessage("Error HDV 1: Por favor, contacta a un administrador.");
              return;
            }
            if(count<=0) {
            	 this.player.sendMessage("Error HDV 2: Por favor, contacta a un administrador");
              return;
            }
          ...
          case '+'://Mettre un objet en vente
            ...
            {
              SocketManager.GAME_SEND_MESSAGE(this.player,"Error HDV 2: Por favor, contacta a un administrador");
              return;
            }
            ...
            catch(Exception e)
            {
              ...
              e.printStackTrace();
              SocketManager.GAME_SEND_MESSAGE(this.player,"Ocurrió un error al vender tu artículo. Por favor, reconéctate para resolver el problema.");
              return;
            }
            if(amount > 3 )
                return;
      ...
      case ExchangeAction.CRAFTING:
        ...
        if(close) {
        	GameClient.leaveExchange(this.player);
	    		...
	    		SocketManager.PACKET_POPUP_DEPART(this.player, "Todos tus objetos fabricados han sido creados.");
      ...
      case ExchangeAction.IN_BANK:
        switch(packet.charAt(2))
        {
          case 'G'://Kamas
            ...
            {
              Main.world.logger.error("Error en el Intercambio Bancario'"+packet+"' => "+e.getMessage());
              e.printStackTrace();
              return;
            }
            if(kamas==0)
      ...
      case ExchangeAction.IN_TRUNK:
        ...
        switch(packet.charAt(2))
        {
          case 'G'://Kamas
            ...
            {
              Main.world.logger.error("Error en el intercambio del cofre '"+packet+"' => "+e.getMessage());
              e.printStackTrace();
              return;
            }
  ...
  private void offlineExchange()
  {
    ...
    if(taxe<0)
    {
      SocketManager.GAME_SEND_MESSAGE(this.player,"Error en modo comerciante: el impuesto es negativo.");
      return;
    }
        ...
        case 'C':// Certificats -> Etable
          ...
          boolean can = false;
			...
			if(this.player.getId() != mount.getOwner() && !can){
				SocketManager.GAME_SEND_MESSAGE(this.player,"¡Estás intentando usar una montura que no pertenece a este personaje!");
			return;	
			}
          mount.setEtape(1);
		  mount.setNumber(1);
          ...
          if(this.player.getMount()!=null&&this.player.getMount().getId()==id)
          {
            ...
            if(mount.getObjects().size()==0)
            {
  			...
  			if(this.player.getId() != mount.getOwner() && !can4){
  				SocketManager.GAME_SEND_MESSAGE(this.player,"¡No puedes montar una criatura que no pertenece a tu personaje!");
  			return;	
  			}
  			if(mount.getEtape() != -1){
  ...
  private void request(String packet)
  {
    ...
    { // Craft sé¿½curisé¿½ : celui qui é¿½ le job ( this.player ) souhaite invité¿½ player
      try
      {
        ...
        if(!ok)
        {
          this.player.sendMessage("Estás demasiado lejos del taller.");
          return;
        }
      ...
      case '0'://Si NPC
        ...
        if(npc!=null)
        {
            ...
            if(npc.getTemplate().getId()==30032) {
            	  SocketManager.PACKET_POPUP_DEPART(this.player, "¡Las compras se realizan con Pévétons!");
              }
  ...
  private void bookOfArtisant(String packet)
  {
    switch(packet.charAt(2))
    {
      case 'F':
        ...
        if(cant==0)
          SocketManager.GAME_SEND_MESSAGE(this.player,"Actualmente no hay ningún artesano disponible en la profesión que buscas.");
        break;
    }
  }
...
private void MorphitemChange(final String packet){
	...
	if (!verif.contains("|")){
		SocketManager.PACKET_POPUP_DEPART(this.player, "¡Ocurrió un error!");
		return;
	}
		final int stats = Integer.parseInt(verif.replace("|", "-").split("-")[0]);
		final int apparence = Integer.parseInt(verif.replace("|", "-").split("-")[1]);
		if (stats == apparence){
			SocketManager.PACKET_POPUP_DEPART(this.player, "¡Debes seleccionar dos objetos diferentes!");
			return;
		}
		final GameObject obj = World.getGameObject(stats);
		if(obj == null){
			SocketManager.PACKET_POPUP_DEPART(this.player, "¡Los dos objetos no pueden ser vinculados!");
			return;
		}
...
//Objeto morphitem = T.createNewMorphItem(T.getID(), obj);
...
/*if(morphitem != null && _perso.addObjet(morphitem, true)){
    ...
    SocketManager.GAME_SEND_POPUP(_perso, "¡Ocurrió un error!");
    return;
}*/
...
//Database.getStatics().getObjectData().update(obj);
		SocketManager.GAME_SEND_ASK(this.player.getAccount().getGameClient(), this.player);
		SocketManager.PACKET_POPUP_DEPART(this.player, "¡Tu objeto ha sido transformado con éxito!");
		player.removeByTemplateID(4,1);
		SocketManager.GAME_SEND_SPELL_LIST(player);
	}
	else {
		SocketManager.PACKET_POPUP_DEPART(this.player, "No tienes Mimibiote contigo");
	}
}
/*
...
private void succes(String packet)
{
	...
	if(Config.singleton.serverId == 6) {
		SocketManager.GAME_SEND_MESSAGE(this.player,""
						+ "Victorias PvM: <b>" + World.get_Succes(this.getPlayer().getId()).getCombat() + "</b>. "
						+ "Victorias PvP: <b>" + World.get_Succes(this.getPlayer().getId()).getPvp() + "</b>. "
						+ "Derrotas PvP: <b>" + World.get_Succes(this.getPlayer().getId()).getPvp_lose() + "</b>. "
						+ "Victorias Koliseo: <b>" + World.get_Succes(this.getPlayer().getId()).getKoli_wine() + "</b>. "
						+ "Derrotas Koliseo: <b>" + World.get_Succes(this.getPlayer().getId()).getKoli_lose() + "</b>. "
						+ "Mazmorras completadas: <b>" + World.get_Succes(this.getPlayer().getId()).getDonjon() + "</b>. "
						+ "Puntos de tienda gastados: <b>" + World.get_Succes(this.getPlayer().getId()).getBoutique() + "</b>. "
						+ "Desafíos ganados: <b>" + World.get_Succes(this.getPlayer().getId()).getChall() + "</b>. "
						+ "Mensajes enviados: <b>" + World.get_Succes(this.getPlayer().getId()).getMsg() + "</b>",
				"008000");
	} else {
		SocketManager.GAME_SEND_MESSAGE(this.player,""
						+ "Combates PvM ganados: <b>" + World.get_Succes(this.getPlayer().getId()).getCombat() + "</b>. "
						+ "Combates PvP ganados: <b>" + World.get_Succes(this.getPlayer().getId()).getPvp() + "</b>. "
						+ "Misiones completadas: <b>" + World.get_Succes(this.getPlayer().getId()).getQuete() + "</b>. "
						+ "Mazmorras completadas: <b>" + World.get_Succes(this.getPlayer().getId()).getDonjon() + "</b>. "
						+ "Archimonstruos derrotados: <b>" + World.get_Succes(this.getPlayer().getId()).getArchi() + "</b>. "
						+ "Puntos de tienda gastados: <b>" + World.get_Succes(this.getPlayer().getId()).getBoutique() + "</b>. "
						+ "Recursos recolectados: <b>" + World.get_Succes(this.getPlayer().getId()).getRecolte() + "</b>. "
						+ "Objetos fabricados: <b>" + World.get_Succes(this.getPlayer().getId()).getCraft() + "</b>. "
						+ "Objetos destruidos: <b>" + World.get_Succes(this.getPlayer().getId()).getBrisage() + "</b>. "
						+ "Runas utilizadas: <b>" + World.get_Succes(this.getPlayer().getId()).getFm() + "</b>. "
						+ "Victorias en Koliseo: <b>" + World.get_Succes(this.getPlayer().getId()).getKoli_wine() + "</b>. "
						+ "Desafíos completados: <b>" + World.get_Succes(this.getPlayer().getId()).getChall() + "</b>. "
						+ "Pisos superados: <b>" + this.getPlayer().Song + "</b>. "
						+ "Mensajes enviados: <b>" + World.get_Succes(this.getPlayer().getId()).getMsg() + "</b>",
				"008000");
	}
}
...
private void batlle_pass () {
  ...
  public void Koli_send_information() {//Inicia server
	...
	public void Koli_add() {
		if(this.getPlayer().getLevel() < 49) {
			SocketManager.GAME_SEND_MESSAGE(this.player,"Nivel 50 requerido", "FF0000");
			return;
		}
		if(Main.world.addKoli_players(this.getPlayer()))
		{
			SocketManager.GAME_SEND_MESSAGE(this.player,"Te has inscrito en el Kolizeum", "008000");
		}
		else
		{
			SocketManager.GAME_SEND_MESSAGE(this.player,"Ya estás inscrito en el Kolizeum", "FF0000");

		}
	}
	public void Koli_remove() {
		if(Main.world.removeKoli_players(this.getPlayer()))
			SocketManager.GAME_SEND_MESSAGE(this.player,"Has abandonado el Kolizeum", "FF0000");
	}
